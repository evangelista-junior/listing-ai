# 💻 listingengine.ai

## About the Project

This repository hosts **Listing Engine**, an AI-empowered tool designed to streamline the creation of high-quality, descriptive listings for **e-commerce products**.

Built using the modern web stack, this application leverages the power of **Artificial Intelligence** to transform minimal photo input into comprehensive and engaging content. It aims to reduce manual effort and increase the efficiency of content creation for listings.

## 🔗 Live Demo

You can view the deployed version of the application here:
[listing-ai-gamma.vercel.app](https://listing-ai-gamma.vercel.app)

## ⚙️ Key Technologies

The project is built on a robust and scalable foundation using the following main technologies:

- **React:** For building the user interface.
- **Next.js (App Router):** The React framework for production, enabling server-side rendering and static site generation.
- **TypeScript:** Ensuring type safety and better code maintainability.
- **[OpenAI]:** Used for content generation and intelligence.
- **Vercel:** Used for deployment and hosting.
- **Tailwind CSS:** For styling and responsiveness.

## 🚀 Getting Started

Follow these steps to set up and run the project locally.

### Prerequisites

You need to have Node.js and a package manager (npm, yarn, pnpm, or bun) installed on your system.

### Installation

1.  Clone the repository:
    ```bash
    git clone [https://github.com/evangelista-junior/listing-ai.git](https://github.com/evangelista-junior/listing-ai.git)
    cd listing-ai
    ```
2.  Install the dependencies:
    ```bash
    npm install
    # or
    yarn install
    # or
    pnpm install
    # or
    bun install
    ```
3.  **Environment Variables Setup:**
    Create a file named `.env.local` in the root of the project and add your necessary API keys and configuration settings.

    ```
    # Example:
    # NEXT_PUBLIC_AI_API_KEY=your_secret_key
    ```

### Running the Development Server

Execute the following command to start the application:

```bash
npm run dev
# or
yarn dev
# or
pnpm dev
# or
bun dev
```
